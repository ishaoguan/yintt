...
class Controller_AvatarFlashUpload extends Controller_Base{
    ...
    public function showuploadAction() {
        ...
        $uc_avatarflash = $this->config->uc_api.'/images/camera.swf?nt=1&inajax=1&input='.$uc_input.'&agent='.md5($_SERVER['HTTP_USER_AGENT']).'&ucapi='.urlencode($this->config->uc_api. '/upload.php' ).'&uploadSize='.$this->config->uploadsize;
        if( $returnhtml == 1 ) {
