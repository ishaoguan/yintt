...
jQuery.extend(ui.box, {
	...
	setcontent:function(content){
		if(content.indexOf("\"status\":0")>=0){
			$jsondate = $.parseJSON(content);
			$('#tsbox_content').text($jsondate.info);
		}
		else
	...
	load:function(data,option,sendType,sendData){
	   var sendType=sendType||"GET";
	   this.init(option);
       var ajax = {
               url: data, type: sendType, dataType: 'html', cache: false, success: function(html) {
