...
class InvestAction extends HCommonAction {
    public function index(){
		...
		if(!empty($maprow['repayment_type'])) $searchMap['b.repayment_type'] =array("in", array_map(intval, $maprow['repayment_type']));
		if(!empty($maprow['borrow_use'])) $searchMap['b.borrow_use'] =array("in", array_map(intval, $maprow['borrow_use']));
		...
		if(!empty($maprow['is_reward'])) $searchMap['b.reward_type'] =array("in", array_map(intval, $maprow['is_reward']));
    ...
    public function detail(){
        ...
        $vminfo = M('members')->field("user_leve,time_limit")->find($this->uid);
        if(!($vminfo['user_leve']>0 && $vminfo['time_limit']>time())) $this->error("请先通过VIP审核再投资",__APP__."/member/vip/invest");
	...
	public function investmoney(){
		...
		$money = intval($_POST['money']);
		$borrow_id = intval($_POST['borrow_id']);
		$amoney = M("member_money")->getFieldByUid($this->uid,'account_money');
		$uname = session('u_user_name');
		...
		if($done===true) $this->success("恭喜成功投标{$money}元");
		elseif($done) $this->error($done);
		else $this->error("对不起，投标失败，请重试!");
	}
	...
	public function ajax_invest(){
		...
		$id=intval($_GET['id']);
		$Bconfig = require C("APP_ROOT")."Conf/borrow_config.php";
		$vo = M('borrow_info')->field("*")->find($id);
		...
		$vo['progress'] = ceil($vo['has_borrow']/$vo['borrow_money']*100);
		...
		$this->assign("vo",$vo);
		$this->assign("account_money",$vm['account_money']);
		$this->assign("Bconfig",$Bconfig);
	...
	public function ajax_vouch(){
		...
		$id=intval($_GET['id']);
		$Bconfig = require C("APP_ROOT")."Conf/borrow_config.php";
		$field = "id,borrow_uid,borrow_money,has_borrow,borrow_interest_rate,borrow_duration,repayment_type,collect_time,has_vouch,reward_vouch_rate";
		$vo = M('borrow_info')->field($field)->find($id);
		
		$vo['need'] = $vo['borrow_money'] - $vo['has_borrow'];
		$vo['lefttime'] =$vo['collect_time'] - time();
		$vo['progress'] = ceil($vo['has_borrow']/$vo['borrow_money']*100);
		$vo['vouch_progress'] = ceil($vo['has_vouch']/$vo['borrow_money']*100);
