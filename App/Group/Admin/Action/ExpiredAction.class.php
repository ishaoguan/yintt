...
class ExpiredAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$field = "m.user_name,d.borrow_id as id,b.borrow_name,d.status,d.total,d.borrow_id,b.borrow_uid,d.sort_order,sum(d.capital) as capital,sum(d.interest) as interest,sum(d.substitute_money) as substitute_money,d.deadline";
    ...
    public function member()
    {
		...
		$map['_string'] = ' d.repayment_time=0 AND d.deadline<'.time().' AND d.status in(0,4) and d.deadline!=0';
		if($_REQUEST['uname']){
			if($_REQUEST['uid']){
				$map['d.borrow_uid'] = $_REQUEST['uid'];
				$search['uid'] = $map['d.borrow_uid'];	
