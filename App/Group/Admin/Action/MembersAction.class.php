    ...
    public function moneyedit()
{
    ...
    public function invite(){
        ...
        $this->assign('award_invest',$award_invest);
        $award_invest=$award_invest/1000;
        import("ORG.Util.Page");
        $map = array();
        $where['user_name'] = text($_REQUEST['user_name']);
        $where['recommend_uname'] = text($_REQUEST['recommend_uname']);

        $map['m.recommend_id'] = array("egt", 1);
        $where['user_name'] && $map['m.user_name']=$where['user_name'];
        $where['recommend_uname'] && $map['m2.user_name']=$where['recommend_uname'];

        $pre = C('DB_PREFIX');
        $suffix = C("URL_HTML_SUFFIX");

        //$count = M('members m')->where($map)->count('m.id');
        $count= M('members')->alias("m")->join("{$pre}members m2 ON m.recommend_id=m2.id")->where($map)->count('m.id');
       /// $parm['pagesize'] = 10000;
        $p = new Page($count, $parm['pagesize']);
        $page = $p->show();
        $Lsql = "{$p->firstRow},{$p->listRows}";
        $orderby = "m.id desc";
        $field = "m.*,m2.user_name as recommend_uname";

        $list = M('members')->alias("m")->field($field)->join("{$pre}members m2 ON m.recommend_id=m2.id")->where($map)->order($orderby)->limit($Lsql)->select();
        $nlist = array();
        $ss=0;
        foreach($list as $mb){
            $rmid = $mb['id'];
            $Model = new Model();
//			$rret = $Model->query("select SUM(investor_capital*borrow_duration) AS tmoney from lzh_borrow_investor as t1
//									left join lzh_borrow_info as t2 on t1.borrow_id=t2.id
//									where t2.borrow_status in (6,7) and investor_uid=$rmid and repayment_type!=1");
//			$rret1 = $Model->query("select SUM(investor_capital*borrow_duration/30) AS tmoney from lzh_borrow_investor as t1
//									left join lzh_borrow_info as t2 on t1.borrow_id=t2.id
//									where t2.borrow_status in (6,7) and investor_uid=$rmid and repayment_type=1");
//			$mb['tmoney'] = ($rret[0]['tmoney']+$rret1[0]['tmoney'])*0.002;

            $rret = $Model->query("select SUM(investor_capital) AS tmoney from lzh_borrow_investor as t1
									left join lzh_borrow_info as t2 on t1.borrow_id=t2.id
									where t2.borrow_status in (6,7) and investor_uid=$rmid ");

            $mb['tmoney']= round(($rret[0]['tmoney']*$award_invest),2);
            $ss=$ss+$mb['tmoney'];
            $nlist[] = $mb;
        }
        $ret = M('members')->getLastSql();
        //var_dump($ret);
        //var_dump($list);exit;
       // echo $ss;
