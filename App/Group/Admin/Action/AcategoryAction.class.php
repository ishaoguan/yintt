...
class AcategoryAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$this->_list(D('Acategory'),$field,array("parent_id"=>0),'sort_order');
        $this->display();
    }
	...
	public function _doAddFilter($m){
		...
		$m->add_time=time();
		R("Admin/Global/cleandatafornoecho");
		return $m;
	}
	...
	public function doAddMul(){
        ...
        if($newid) $this->success("栏目批量添加成功");
    ...
    public function listType()
    {
		...
		$sonlist = D('Acategory')->field(true)->order('sort_order desc')->where("parent_id={$typeid}")->select();
