    ...
    */
	...
	public function chargeexport(){
		...
		foreach($list as $v){
				...
				$row[$i]['card_6'] = $payTypes[$v['way']];
				$row[$i]['card_7'] = date("Y-m-d H:i",$v['add_time']);
				$i++;
		}
		
		$xls = new Excel_XML('UTF-8', false, 'datalist');
		$xls->addArray($row);
		$xls->generateXML("chargerecordlist");
	}
    ...
    public function withdraw()
    {
		...
		$field= 'w.*,m.user_name,m.id mid';
		$list = M('member_withdraw w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->where($map)->limit($Lsql)->order("w.id DESC")->select();
	...
	public function withdrawexport(){
		...
		$field= 'w.*,m.user_name,m.id mid';
		$list = M('member_withdraw w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->where($map)->limit($Lsql)->select();
		...
		foreach($list as $v){
				...
				$row[$i]['card_3'] = $v['withdraw_fee'];
				$row[$i]['card_4'] = $status[$v['withdraw_status']];
				$row[$i]['card_5'] = date("Y-m-d H:i",$v['add_time']);
				$row[$i]['card_6'] = ($v['deal_time']>0)?date("Y-m-d H:i",$v['deal_time']):"未处理";
