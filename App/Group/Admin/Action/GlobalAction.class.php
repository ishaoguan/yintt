...
class GlobalAction extends ACommonAction
{
    ...
    */
	...
	public function cleandatafornoecho(){
		$dirs	=	array(C('APP_ROOT').'Runtime/Temp');
		foreach($dirs as $value)
		{
			rmdirr($value);
    ...
    public function cleangame()
    {
		...
		$dirs	=	array(C('APP_ROOT').'gamedata');
		foreach($dirs as $value)
		{
			rmdirr($value);
			echo "<div style='border:2px solid green; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:green;text-align:center;'>\"".$value."\" 目录下缓存清除成功! </div> <br /><br />";
		...
		for($i=65;$i<=90;$i++){
			$first_le = strtolower(chr($i));
			$game_list_data = M('games')->field('game_py,game_name,id')->where("left(game_py,1)='$first_le'")->select();
			creatfile($first_le,serialize($game_list_data));
			echo "<div style='border:2px solid green; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:green;text-align:center;'>正在生成".chr($i)."字母游戏缓存</div>";
		}
