...
class ACommonAction extends Action
{
	...
	var $admin_id = 0;
	var $pre = NULL;
	var $logOpType = NULL;//日志操作类型
	var $logUserType = NULL;//日志操作人员类型
	//验证身份
	function _initialize(){
		...
		if(strtolower(MODULE_NAME) == 'index' && (strtolower(ACTION_NAME) == 'login' || (strtolower(ACTION_NAME) == 'index' && !session('admin') ))
				&& $_SERVER['PATH_INFO'] != '/'.SAFE_ADMIN){
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
				...
				if (false !== $model->where("{$pk} in ({$id})")->delete()) {
					$this->success(L('删除成功'),'',array("data"=>$id));
				} else {
					$this->error(L('删除失败'));
				}
            } else {
