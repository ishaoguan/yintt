<?php
class NewBorrowAction extends ACommonAction
{
	var $smsLimit  =1000;
	
	public function index(){
        $count = M('sms_content')->alias('t')->where(array("t.sms_type"=>'NEW_BORROW'))->count("1");
        import("ORG.Util.Page");
        $p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		$list = M('sms_content')->alias('t')->join("left join {$this->pre}borrow_info bi on t.borrow_id=bi.id")->
        	join("left join {$this->pre}ausers au on au.id=t.deal_user")->
        	field("t.*,bi.borrow_name,au.real_name deal_user_name,(select count(1) from {$this->pre}sms_receiver sr where sr.sms_log_id=t.id and sr.sms_status='0') wait_send_count,
			...
			where(array("t.sms_type"=>'NEW_BORROW'))->limit($Lsql)->order("t.id DESC")->select();
