...
class MemberdataAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		//print_r(M('member_data_info mf')->getLastSql());
	...
	public function doEdit(){
        ...
        if ($result = $model->save()) { //保存成功
			...
			memberCreditsLog($vd['uid'],1,intval($this->glo["file_exp"]), $vd['data_name'].'审核通过奖励经验');
			memberScoresLog($vd['uid'],1,intval($this->glo["file_score"]), $vd['data_name'].'审核通过奖励积分');
            //成功提示
