...
class CapitalDetailAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$field= 'l.id,l.add_time,m.user_name,l.affect_money,l.freeze_money,l.collect_money,l.account_money,l.target_uname,l.type,l.info,m.id mid';
		$order = "l.id DESC";
		$list = M('member_moneylog')->alias('l')->field($field)->join("{$this->pre}members m ON m.id=l.uid")->where($map)->limit($Lsql)->order($order)->select();
	...
	public function export(){
		...
		$type = C('MONEY_LOG');
		$row=array();
		$row[0]=array('序号','用户ID','用户名','交易对方','交易类型','影响金额','可用余额','冻结金额','待收金额','发生时间','说明');
		$i=1;
		foreach($list as $v){
				$row[$i]['i'] = $i;
				$row[$i]['uid'] = $v['mid'];
				...
				$row[$i]['card_timelimit'] = date("Y-m-d H:i",$v['add_time']);
				$row[$i]['card_info'] = $v['info'];
				$i++;
		}
		
		$xls = new Excel_XML('UTF-8', false, 'datalist');
		$xls->addArray($row);
		$xls->generateXML("capitalrecordlist");
	}
}
?>