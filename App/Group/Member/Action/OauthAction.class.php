...
class OauthAction extends MCommonAction {
	...
	public function qq(){
		...
		//echo $_SESSION["appid"].'---'.$_SESSION["scope"].$cb;
...
// 		$sinauser['name'] = sprintf("%05d",rand(1,99999));
		...
		$map['site'] = 'sina';
		$this->appCk($map, $sinauser['name']);//nickname
	}
	...
	private function appCk($map,$nickname){
		$vo = M('oauth')->field('id,logintimes,is_bind,bind_uid,login_enable')->where($map)->find();
		if(is_array($vo)){//以前登陆过
			//该登陆方式已关闭？
			if($vo["login_enable"] != 1){
				$this->error("该登陆方式已被用户禁用，不允许登陆！", "__APP__/member/common/login/");
				return;
			}
...
// 				$this->display('index');
			...
			if(isset($map["bind_uid"])){
				$this->applogin($map['bind_uid']);
				$url_referer = session("url_referer");
				if(!empty($url_referer)){
					session("url_referer", null);
					redirect($url_referer);
	...
	private function logapp($data){
		...
		$save['logintime'] = time();
		if(isset($data["is_bind"]))$save['is_bind']=$data["is_bind"];
		M('oauth')->save($save);
	}
	...
	private function addapp($data){
		...
		$count = 0;
		while(true){
			$remember['user_name'] = "@{$data['site']}_".$data['nickname'].sprintf("%05d",rand(1,99999));
			$map['user_name']  = array('eq',$remember['user_name']);
			$memberCount = M()->table($this->memberTable)->where($map)->count();
			if($memberCount == 0){
				break;
			}
			++$count;
