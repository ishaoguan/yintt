...
class NewtipAction extends MCommonAction {
    ...
    public function newtip(){
		...
		if(empty($sqs['phone_status'])) $data['html'] = '<script type="text/javascript">alert("您还未验证手机，请先验证");window.location.href="'.__APP__.'/member/verify#fragment-2";</script>';
		else{
			$vo = M('borrow_tip')->where("uid={$this->uid}")->find();
			$vo['borrow_type'] = ($vo['borrow_type']==3)?"流转标":"普通标";
