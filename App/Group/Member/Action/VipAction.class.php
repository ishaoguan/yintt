...
class VipAction extends MCommonAction {
    ...
    public function index(){
    	$vf =M('members_status')->field('phone_status,id_status')->find($this->uid);

//      	if($vf['phone_status']!=1){
//  			$this->error("您还没有通过手机认证","/member/verify#fragment-2");
//      	}
//    	if($vf['id_status']!=1){
//    		$this->error("您还没有通过实名验证","/member/verify#fragment-3");
//    	}
...
//      	}
        if($vf['phone_status']!=1){
            $this->error("您还没有通过手机验证","/member/verify#fragment-2");
        }else{
            $this->index();
        }
    }
