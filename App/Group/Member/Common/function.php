...
function getChargeLog($map,$size,$limit=10){
	...
	foreach($list as $key=>$v){
		...
		$list[$key]['way_name'] = $payType[$v['way']];
	}
...
function getBorrowList($map,$size,$limit=10){
	...
	foreach($list as $key=>$v){
		...
		if($map['borrow_status']==6){
			$vx = M('investor_detail')->field('deadline')->where("borrow_id={$v['id']}")->order("deadline ASC")->find();
...
function getTenderList($map,$size,$limit=10, $orderby='i.id DESC'){
	$pre = C('DB_PREFIX');
	$Bconfig = require C("APP_ROOT")."Conf/borrow_config.php";
	...
	if($size){
		//分页处理
		import("ORG.Util.Page");
		$count = M('borrow_investor')->alias("i")->where($map)->count('i.id');
		$p = new Page($count, $size);
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
	...
	$field = "distinct i.*,i.add_time as invest_time,m.user_name as borrow_user,b.borrow_duration,b.has_pay,b.borrow_interest_rate,b.add_time as borrow_time,b.borrow_money,b.borrow_name,m.credits,b.repayment_type,b.borrow_type,id.repayment_time,DATEDIFF(DATE_FORMAT(FROM_UNIXTIME(id.repayment_time),'%Y-%m-%d'), DATE_FORMAT(FROM_UNIXTIME(i.add_time),'%Y-%m-%d')) invest_days,b.second_verify_time";
	$list = M('borrow_investor')->alias("i")->field($field)->where($map)->join("{$pre}borrow_info b ON b.id=i.borrow_id")->join("{$pre}members m ON m.id=b.borrow_uid")->
		join("left join {$pre}investor_detail id ON id.invest_id=i.id and id.investor_uid=i.investor_uid")->order($orderby)->limit($Lsql)->select();
	foreach($list as $key=>$v){
		if($map['i.status']==4){
	...
	$row['list'] = $list;
	$row['page'] = $page;
	$row['total_money'] = M('borrow_investor')->alias("i")->where($map)->sum('investor_capital');
	$row['total_num'] = $count;
	return $row;
}
