...
<div class="so_main">
  ...
  <div id="addAttr_div" style="display:none;">
    ...
    <dl class="lineD" id="game_friend" style="display:none">
      <dt>游戏名称：</dt>
      <dd>
        <input name="game_name" id="game_name" class="input" type="text" value="" onkeyup="show(event);" autocomplete="off" disabled="disabled">
        <span id="tip_game_name" class="tip">输入游戏拼音首字母或者汉字，会有下拉提示，可在下拉列表中选择</span>
      </dd>
    </dl>
...
var show_sn;
 ...
 <script type="text/javascript">
 function showgame(){
 	var se = $("#link_type option:selected").text();//
	if(se == "游戏专题"){
		$("#game_friend").show();
...
<!--添加友情链接-->
  ...
  <div id="searchFriend_div" style="display:none;">
    ...
    <dl class="lineD" id="game_friend_search">
      <dt>游戏名称：</dt>
      <dd>
        <input name="game_name" id="game_name_search" class="input" type="text" value="" autocomplete="off">
        <span id="tip_game_name_friend" class="tip">如果不填则不限制</span>
      </dd>
    </dl>
...
function searchFriend(s) {
  ...
  <?php if(is_array($friend_list)): $i = 0; $__LIST__ = $friend_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="area_<?php echo ($vo["id"]); ?>">
		...
		<td><span id="link_game_<?php echo ($vo['id']); ?>"><?php echo ($vo["game_name"]); ?></span></td>
