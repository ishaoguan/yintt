...
class SaeTOAuthV2 {
	...
	function userShowURL()     { return 'https://api.weibo.com/2/users/show.json'; }
	...
	function getUserShow($uid){
		$params = array();
		$params['uid'] = $uid;
		$params['access_token'] = $this->access_token;
		$response = $this->oAuthRequest($this->userShowURL(), 'GET', $params);
		$user = json_decode($response, true);
		if(is_array($user) && !isset($user['error'])){
			
		} else {
			throw new OAuthException("get user show failed." . $user['error']);
		}
		return $user;
	}
